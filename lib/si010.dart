...
// ★BottomNavの「同じタブなら何もしない」を正しく判定するため
enum _MenuScreenKind { list, profile, other }

class Si010Page extends StatefulWidget {
  // ✅ si009からの遷移で「元の情報（プラン名）」を保持するための入口（最小追加）
  final String? initialPlanTitle;

  const Si010Page({super.key, this.initialPlanTitle});
...
class _Si010PageState extends State<Si010Page> {
  ...
  // ✅ BottomNav：0=一覧 / 1=プロフィール（si010はどちらでもないが表示上の選択）
  int _menuIndex = 0;

  // ★si010は「一覧でもプロフィールでもない」＝ other 固定
  final _MenuScreenKind _currentScreen = _MenuScreenKind.other;
  ...
  void initState() {
    ...
    // ✅ 一覧から来た場合はプラン名を初期反映
    final t = (widget.initialPlanTitle ?? '').trim();
    if (t.isNotEmpty) {
      _planTitleController.text = t;
    }
  ...
  Future<void> _addDay() async {
    ...
    final bool exists = _days.any((d) => _sameDay(d.date, picked));
    if (exists) {
    ...
    setState(() {
      _days.add(DayDraft(date: picked));
  ...
  // ✅ BottomNav（2項目）だけ：一覧 / プロフィール
  Future<void> _onTapMenu(int index) async {
    if (index == 0 && _currentScreen == _MenuScreenKind.list) return;
    if (index == 1 && _currentScreen == _MenuScreenKind.profile) return;
    ...
    Navigator.pushReplacement(
      context,
      MaterialPageRoute(builder: (_) => const screen007.Screen007Page()),
    );
  }
